    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                ...
                [
                    InlineKeyboardButton(text="•  •", callback_data="mat"),
            ]
        ]
    )
    if m.sender_chat:
...
async def vstream(c: Client, m: Message):
    ...
    try:
        ...
        if m.chat.username:
            try:
                ...
                await m.reply_text(f"âŒ **userbot failed to join**\n\n**reason**: `{e}`")
                return
        else:
            try:
                user_id = (await user.get_me()).id
    ...
    if len(m.command) < 2:
        ...
        if veez == 0:
            await loser.edit(f"âŒ yt-dl issues detected\n\nÂ» `{livelink}`")
        else:
            if chat_id in QUEUE:
